...
function auToggle() {
  ...
  if (AU.on) { auStart(); if (AU.songId) { const id = AU.songId; AU.songId = null; playSong(id); } }
  return AU.on;
}
// おと 1つ
function tone(o) {
...
function playSong(id) {
  if (!AU.on) { AU.songId = id; return; }
  auStart();
  if (!AU.ctx) return;
  if (AU.songId === id && AU.timer) return;
  stopSong();
  AU.songId = id; AU.song = SONGS[id]; AU.step = 0;
  if (!AU.song) return;
...
['pointerdown', 'touchstart', 'click'].forEach(ev =>
  document.addEventListener(ev, function once() {
    auStart();
    if (AU.songId && !AU.timer) { const id = AU.songId; AU.songId = null; playSong(id); }
...
function startBattle() {
  ...
  showPanel('hand');
  const e = makeEnemy();
  playSong(songFor(e.boss ? 'boss' : 'battle'));
